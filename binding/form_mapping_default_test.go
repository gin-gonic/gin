//go:build !gin_bind_default_encoding_text_unmarshaler
...
// If someone does not specify parser=encoding.TextUnmarshaler even when it's defined for the type, gin should ignore
// the UnmarshalText logic and continue using its default binding logic. (This ensures gin does not break backwards
// compatibility)
//
// Note: TestMappingUsingBindUnmarshalerAndTextUnmarshalerWhenOnlyTextUnmarshalerDefined works differently when:
// - form_mapping_encoding_test.go (with gin_bind_default_encoding_text_unmarshaler build tag enabled)
