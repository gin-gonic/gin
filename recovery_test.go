...
func TestSecureRequestDump(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantContains != "" && !strings.Contains(strResult, tt.wantContains) {
				t.Errorf("maskHeaders() = %q, want contains %q", strResult, tt.wantContains)
			}
			if tt.wantNotContain != "" && strings.Contains(strResult, tt.wantNotContain) {
				t.Errorf("maskHeaders() = %q, want NOT contain %q", strResult, tt.wantNotContain)
			}
		})
	}
}
